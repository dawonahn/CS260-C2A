...
def IPOT_distance_torch_batch_uniform(C, bs, n, m, iteration=50, device='cuda:2'):
	C = C.float().to(device)
...
def IPOT_distance_torch_batch_uniform_T(C, bs, n, m, iteration=50, device='cuda:2'):
	C = C.float().to(device)
	T = IPOT_torch_cost_matrix_batch_torchbatch_uniform(C, bs, n, m, iteration=iteration)
	# temp = torch.bmm(torch.transpose(C,1,2), T)
	# distance = batch_trace(temp, m, bs)
	return T


def IPOT_torch_batch_uniform(C, bs, n, m, beta=0.5, iteration=50,device='cuda:2'):
	# C is the distance matrix
	# c: bs by n by m
...
def GW_distance(X, Y, p, q, lamda=0.5, iteration=5, OT_iteration=20, device='cuda:2'):
	'''
...
def GW_torch_batch(Cs, Ct, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=20, device='cuda:2'):
...
# def GW_torch_batch(Cs, Ct, bs, n, m, beta=0.5, iteration=5, OT_iteration=20, device='cuda:2'):
# 	one_m = torch.ones(bs, m, 1).float().to(device)
# 	one_n = torch.ones(bs, n, 1).float().to(device)
...
def GW_distance_uniform(X, Y, lamda=1e-1, iteration=5, OT_iteration=20, device='cuda:2'):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	p = (torch.ones(bs, m, 1)/m).to(device)
...
def batch_trace(input_matrix, n, bs, device='cuda:2'):
